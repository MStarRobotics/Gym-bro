...
async def test_broadcast_trainer_status_logs(caplog):
    ...
    await service.broadcast_trainer_status("trainer-1", "online", {"uptime": 100})
...
async def test_connect_and_broadcast_timestamp(caplog):
    ...
    evt = await service.broadcast_trainer_status("trainer-1", "online", {"uptime": 1})
    assert isinstance(evt.get("timestamp"), str)
    assert "trainer_id" in evt
    # booking update payload has booking_id and timestamp
    booking_evt = await service.broadcast_booking_update("booking-1", "confirmed")
...
async def test_subscribe_trainer_logs(caplog):
    ...
    assert any("SOCKET_SUBSCRIBE" in rec.getMessage() for rec in caplog.records)
